...
| **學生端本地 Snapshot** | **`TCP 8080` (或隨機分配)** | 學生端內建微型 HTTP 伺服器，提供 `/snapshot` 縮圖調閱 |
...
2. 建立 Git Tag 並推送：
   ```bash
   git tag -a v5.3.1 -m "Release v5.3.1: Student fast join portal, universal HTTP clipboard copy, standalone gs-console.exe"
