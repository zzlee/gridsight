...
[![Architecture: v5.9.0](https://img.shields.io/badge/Spec-v5.9.0-emerald.svg)](docs/architecture.md)

**GridSight** 專為具備還原卡之 Windows 電腦教室打造，兼顧「實體座位自由排版」、「全班畫面低負載輪詢」、「單機 30 FPS 焦點調閱」與「教師畫面全體廣播」四大核心功能。

> 💡 **第一次使用？請直接參閱：[📖 GridSight 全功能無腦使用手冊 (Zero-Brain User Manual)](docs/wiki/user-manual.md)**，包含全系統 15 大功能的「3 步驟防呆操作」與快速鍵總覽！
...
- 🔒 **輕量無感部署與安全鑑權 (Zero-Maintenance & Security)**
  ...
  - **動態點名簽到**：教師按需發起點名，學生端彈出原生 Win32 對話框輸入學號，即時簽到名冊維護與 CSV 匯出
