...
export const FocusModal: React.FC<FocusModalProps> = ({ device, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 select-none">
      ...
      >
        {/* 1. OS-Style Window Title Bar (最上方視窗標題列) */}
        <div className="h-8 bg-slate-950 border-b border-slate-800/80 px-3 flex items-center justify-between select-none shrink-0 gap-2">
          {/* Left: Window Icon & Active Window Title */}
          <div className="flex items-center space-x-2 min-w-0 flex-1">
            ...
            <span className="text-xs font-semibold text-slate-200 truncate">
              {device.activeWindow || 'Windows 桌面'}
            </span>
            {device.isOffTask && (
              <span className="px-1.5 py-0.2 rounded bg-rose-600/30 text-rose-300 border border-rose-500/40 text-[10px] font-bold whitespace-nowrap shrink-0 animate-pulse">
                ⚠️ 離題程式
              </span>
            )}
            <span className="text-[11px] text-slate-500 font-mono hidden md:inline truncate">
              — [座號 {device.seatNo || '未分配'}] {device.hostname}
            </span>
          </div>

          {/* Right: Window Controls (Fullscreen & Close) */}
          <div className="flex items-center space-x-1 shrink-0">
            ...
            <button
              onClick={onClose}
              className="p-1 rounded text-slate-400 hover:text-white hover:bg-rose-600 transition-colors"
              title="關閉視窗"
            >
        ...
        {/* 2. Action Toolbar (教學操作與管制工具列) */}
        <div className="h-11 bg-slate-900 border-b border-slate-800 px-3 sm:px-4 flex items-center justify-between gap-2 select-none shrink-0">
          {/* Left: Device Info (Seat No, Hostname, IP, Auth) */}
          <div className="flex items-center space-x-2 sm:space-x-3 shrink min-w-0">
            ...
            <span className="font-bold text-slate-100 text-sm sm:text-base whitespace-nowrap shrink-0">
              {device.hostname}
            </span>
          ...
          {/* Right: Actions (Showcase, Lock, Snapshot, Record, Diagnostic) */}
          <div className="flex items-center space-x-1.5 sm:space-x-2 shrink-0">
            {/* Student Showcase Relay Button (Feature 3) */}
            <button
              onClick={handleToggleShowcase}
              className={`p-1.5 rounded-lg border transition-all active:scale-95 flex items-center space-x-1 text-xs font-semibold px-2.5 whitespace-nowrap shrink-0 ${
                isShowcasing
            ...
            >
              ...
              <span>{isShowcasing ? '轉播中 [停止]' : '轉播給全班'}</span>
            </button>
            ...
            >
              ...
              <span>{isDeviceLocked ? '已鎖定 [解鎖]' : '黑屏鎖定'}</span>
            </button>
            ...
            <div className="flex items-center rounded-lg bg-slate-800 border border-slate-700 overflow-hidden shrink-0 text-xs shadow-sm">
              <button
                ...
                className="px-2.5 py-1.5 hover:bg-sky-600 text-slate-300 hover:text-white transition-colors flex items-center space-x-1 whitespace-nowrap"
            ...
            <button
              ...
              className={`p-1.5 sm:px-2.5 py-1.5 rounded-lg border transition-all active:scale-95 flex items-center space-x-1 text-xs font-semibold whitespace-nowrap shrink-0 ${
                isRecordingStudent
            ...
            <div ref={toolMenuRef} className="relative shrink-0">
              <button
                ...
                className={`p-1.5 sm:px-2.5 py-1.5 rounded-lg border text-xs font-semibold flex items-center space-x-1 transition-all shadow-sm active:scale-95 whitespace-nowrap ${
