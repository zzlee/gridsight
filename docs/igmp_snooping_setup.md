# 電腦教室 1Gbps LAN 交換器 IGMP Snooping 設定指南

在多台電腦的高密度區域網路環境中，若未正確配置 IGMP Snooping，UDP Multicast 廣播會退化為全端口廣播風暴 (Broadcast Storm)，導致網絡擁塞。
...
2. 全班電腦同時接收廣播時，上行交換器總頻寬僅約 1.5~8 Mbps（依所選品質），全班流暢零延遲。
