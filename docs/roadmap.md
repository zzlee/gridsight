...
- [x] **Task 5.13 學生端原生 30 FPS 串流穩定化、學生焦點錄影落地與獨立歷史錄影庫 (v5.9.0)**：(1) 學生端 Linux/Windows 原生 30 FPS H.264 串流首幀 IDR 暖機與看門狗心跳誤殺修復；(2) 學生焦點畫面串流錄影端到端落地（FFmpeg 零拷貝 muxing、錄影自動協同開流、首幀 IDR 零延遲注入、檢視器關閉串流保護）；(3) 獨立「歷史錄影清單」模組 (`RecordingsListModal.tsx`)，支援分類篩選（教師/廣播/學生）、搜尋、HTML5 `<video>` 串流拖曳預覽（HTTP 206 Partial Content）、下載與刪除；(4) 學生端反向 WebSocket 受控出站與多播探索架構全面穩固。
