...
### 階段 5：廣播多檔位品質、True Alpha 滑鼠特效、課堂進階與音訊錄製 (v5.8.0 ~ v5.8.10)
...
- [x] **Task 5.7 課堂專注黑屏鎖定與示範畫面轉播 (v5.8.9)**：全班/選定學生深藍黑屏與低階鍵鼠攔截鎖定（含斷線重連自動補鎖）；學生示範畫面 30 FPS 零拷貝直推 RTP 多播（含防鏡像遞迴抑制）。
- [x] **Task 5.8 免外網課堂作業收取與自動歸檔 (v5.8.9)**：學生端 Windows 原生 C++ 拖曳視窗（免開瀏覽器）；最新版覆蓋歸檔；零依賴純 Node.js ZIP 打包串流下載。
- [x] **Task 5.9 教師螢幕錄影自選音訊裝置與多播強隔離 (v5.8.10)**：DirectShow / PulseAudio 音效卡與麥克風自動探測；AAC 128kbps 影音精準混流同步 (`aresample=async=1000`)；雙軌多播強隔離（多播廣播嚴格過濾阻斷聲音 `-an` 與 `select=v`，100% 杜絕全班喇叭回音嘯叫，音訊僅寫入本機 MP4）；錄音偏好持久化記憶。
- [x] **Task 5.10 全功能無腦使用手冊與文檔工程標準化 (v5.8.9 ~ v5.8.10)**：建立 14 大功能 3 步驟圖解指南 (`user-manual.md`)、標準通用 HTML 錨點、全套 14 份 Markdown 深度靜態檢查無錯誤。
- [x] **Task 5.11 廣播極致低延遲調優與閉環量測工具 (v5.8.11)**：(1) 教師端 FFmpeg x264 消除 lookahead 緩衝 (`sync-lookahead=0:rc-lookahead=0`)、GOP 0.5 秒、輸入輸出無緩衝，消除 1.3+ 秒編碼積壓；(2) 學生端 `rtp_receiver.cpp` 渲染解碼 `UpdateWindow` 即時繪圖；(3) 多機快照批次聚合端點 (`POST /api/snapshots/batch`) 與 `since` 差量時間戳比對（靜態畫面 0 載荷、React 批次更新）；(4) 新增「⏱️ 廣播延遲量測工具 (Benchmark)」：60 FPS 基準毫秒碼錶 + 250ms 循環色塊 + 學生畫面即時閉環回傳視覺同屏毫秒對比。
- [x] **Task 5.12 介面版號直觀呈現與全專案通用化修訂 (v5.8.12)**：(1) 在教師端 Console 頂部導航列品牌標題旁與「更多工具」下拉選單底部加入系統版號徽章；(2) 全面淡化並通用化專案文檔與介面中對單一固定人數（「70人/70台」）之特定描述，強調通用電腦教室規模與高併發支援。
- [ ] **Task 5.13 探索方向反轉與受控出站架構 (v5.9.0)**：學生端不再定時發送 Beacon 探索包，改為**監聽教師端定期多播「教師在線」探索宣告 (UDP 8888)** 後建立唯一出站反向 WebSocket，後續所有指令與上報皆經由該長連線；上線註冊改以 `AGENT_INFO_REGISTER` 於 WS 握手後第一時間上報（hostname / specs / 座號 / activeWindow）；共用 Session Token（僅存記憶體）隨探索封包派發，教師端改為定期廣播發送端；教師視訊廣播 (9000) 與輸入多播 (9002) 維持不動。
...
### 階段 7：進階課堂互動與考場控制 (未來目標)
- [x] **Task 7.1 學生示範廣播 (Student Showcase Relay)**：【已於 v5.8.9 完成，見 Task 5.7】支援選定學生畫面 30 FPS 直推全班，具備示範機右下角無激活提示與多播防鏡像遞迴抑制。
- [x] **Task 7.2 課堂動態點名與學號簽到系統 (Dynamic Roll Call System)**：【已完成】徹底移除啟動強制彈窗，改為教師端按需發起點名（全班或單機重填），學生端 WS 接收後彈出原生 Win32 對話框輸入學號簽到，即時名冊維護、持久化 attendance.json 與 CSV 匯出。
- [ ] **Task 7.3 課堂即時快問快答與隨堂測驗 (Instant Quiz & Live Polling)**：免開網頁、0 外網依賴，教師端一鍵出單選/是非題，學生端彈出原生置頂卡片點選，10 秒內完成全班統計柱狀圖。
- [ ] **Task 7.4 學生無聲舉手與求助追蹤系統 (Silent Hand-Raise & Help Desk)**：學生端桌面一鍵無聲舉手求助，教師畫布卡片亮黃色跳動脈衝，頂部排隊清單一鍵導航聚焦。
- [ ] **Task 7.5 上機考試防作弊：一鍵外網切斷（保留教師監控通道）(Exam Network Shield)**：考試模式下一鍵阻斷學生端一切外網與跨機通訊，嚴格僅放行往教師 IP 之監控與收作業通道。
- [ ] **Task 7.6 廣播視窗滑鼠自動跟隨 (Auto-Tracking Mouse)**：接收廣播畫面時，視窗 viewport 自動平滑跟隨教師滑鼠座標移動。
- [ ] **Task 7.7 廣播視窗強制全螢幕模式 (Immersive Fullscreen Broadcast)**：教師端廣播時可下發指令強制學生端全螢幕置頂播放，隱藏所有 Windows 工具列。
