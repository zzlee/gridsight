...
- [x] **Task 5.11 廣播極致低延遲調優與閉環量測工具 (v5.8.11)**：(1) 教師端 FFmpeg x264 消除 lookahead 緩衝 (`sync-lookahead=0:rc-lookahead=0`)、GOP 0.5 秒、輸入輸出無緩衝，消除 1.3+ 秒編碼積壓；(2) 學生端 `rtp_receiver.cpp` 渲染解碼 `UpdateWindow` 即時繪圖；(3) 多機快照批次聚合端點 (`POST /api/snapshots/batch`) 與 `since` 差量時間戳比對（靜態畫面 0 載荷、React 70 次降至 1 次批次更新）；(4) 新增「⏱️ 廣播延遲量測工具 (Benchmark)」：60 FPS 基準毫秒碼錶 + 250ms 循環色塊 + 學生畫面即時閉環回傳視覺同屏毫秒對比。
