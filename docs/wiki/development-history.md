...
| **v5.9.0** | 2026-09 | `beacon_client.cpp`→`discovery_listener`, `ws_server.cpp`, `multicastDiscovery.ts`, `tokenAuthority.ts`, `docs/*` | 探索方向反轉與受控出站架構：學生端不再定時發送 Beacon 探索包（改為監聽教師端定期多播「教師在線」宣告 UDP 8888）；學生端僅保留唯一出站長連線 `/ws/agent`，上線改以 `AGENT_INFO_REGISTER` 於握手後第一時間註冊；共用 Session Token（僅存 RAM）隨探索封包派發；教師端由「監聽/回應」改為「定時廣播發送端」；教師視訊廣播 (9000) 與輸入多播 (9002) 維持不動 |
